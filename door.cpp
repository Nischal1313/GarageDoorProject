...
void Door::updateDoorState() {
    if (sw1StateChanged && isCalibrated) {
        ...
        switch (doorState) {
            ...
            mqttManager->publishMessage("garage/door/response", "{ \"response\": \"command completed\", \"action\": \"gate closed\" }");
                break;

            case DOOR_OPEN:
                doorState = DOOR_CLOSING;
                stopMotor = false;
            mqttManager->publishMessage("garage/door/response", "{ \"response\": \"command completed\", \"action\": \"gate opened\" }");
                break;

            case DOOR_CLOSING:
                doorState = DOOR_STOPPED;
            mqttManager->publishMessage("garage/door/response", "{ \"response\": \"command completed\", \"action\": \"gate stopped\" }"); // Response when stopping
                break;
